...
export default function FreelancerProjectPage() {
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      function addText(text: string, size: number, bold = false, color: [number, number, number] = [15, 23, 42]) {
      ...
      if (b.estimated_cost_range?.budget) {
        addText(`Budget: ₹${(b.estimated_cost_range.budget.min / 100000).toFixed(1)}L – ₹${(b.estimated_cost_range.budget.max / 100000).toFixed(1)}L`, 11)
        addText(`Premium: ₹${(b.estimated_cost_range.premium.min / 100000).toFixed(1)}L – ₹${(b.estimated_cost_range.premium.max / 100000).toFixed(1)}L`, 11)
      }
  ...
  const chatAllowed = ['READY_TO_SELECT', 'DEVELOPER_ASSIGNED', 'IN_DEVELOPMENT', 'TESTING', 'COMPLETED'].includes(project.status)
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#020817', color: 'white' }}>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
              <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '16px', padding: '24px' }}>
                <h2 style={{ color: '#64748b', fontSize: '11px', fontWeight: '600', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '12px' }}>Problem Statement</h2>
                <p style={{ color: '#cbd5e1', fontSize: '14px', lineHeight: '1.7' }}>{b.problem_statement}</p>
              </div>
              <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '16px', padding: '24px' }}>
                <h2 style={{ color: '#64748b', fontSize: '11px', fontWeight: '600', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '12px' }}>Target Users</h2>
                <p style={{ color: '#cbd5e1', fontSize: '14px', lineHeight: '1.7' }}>{b.target_users}</p>
              </div>
            </div>

            <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: '16px', padding: '24px', marginBottom: '16px' }}>
              <h2 style={{ color: '#64748b', fontSize: '11px', fontWeight: '600', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '16px' }}>Core Features</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '16px' }}>
              {[
                ...
                { label: 'Timeline', value: `${b.estimated_timeline_days} days`, capitalize: false },
                {
                  label: 'Budget estimate', capitalize: false,
      ...
      {/* Chat panel */}
      {chatOpen && currentUserId && project && (
        <ChatPanel
